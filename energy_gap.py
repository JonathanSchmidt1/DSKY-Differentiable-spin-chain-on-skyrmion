...
def energy_gap_3d(param_values, scalfac = 1.0, J_1 = -1.0, L = 20, n_eigs = 4, dtype = torch.float32, hdf5_path = "data/sweep/sweep3d"):
    pathlib.Path(hdf5_path).mkdir(parents=True, exist_ok=True)
...
def energy_gap_test(param_values, J_1 = -1.0, L = 20, n_eigs = 4, dtype = torch.float32, hdf5_path = "data/testing/"):
    pathlib.Path(hdf5_path).mkdir(parents=True, exist_ok=True)
